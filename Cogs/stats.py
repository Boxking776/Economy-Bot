...
class Stats(BaseCog):
    ...
    async def bottom(self, context, command=None):
        """Shows bottom ten users for a given aspect. If no argument is given, balance is used. Other options: given, received, br_wins, br_score, br_winnings, brs, duel_wins, duel_winnings, duels, races, first_place_bets, race_winnings, gambling_profit."""
        ...
        try:
            ...
            await self.bot.post_message(self.bot.bot_channel, 'There are no users.')
            return
    ...
    async def top(self, context, command=None):
        """Shows top ten users for a given aspect. If no argument is given, balance is used. Other options: given, received, br_wins, br_score, br_winnings, brs, duel_wins, duel_winnings, duels, races, first_place_bets, race_winnings, gambling_profit."""
    ...
    async def all(self, context, command=None):
        """Shows all users in descending order, sorted by a given aspect. If no argument is given, balance is used. Other options: given, received, br_wins, br_score, br_winnings, brs, duel_wins, duel_winnings, duels, races, first_place_bets, race_winnings, gambling_profit."""
    ...
    async def season(self, context, number):
        ...
        if amnt_seasons < 1:
            await self.bot.post_message(self.bot.bot_channel, 'No previous season data is available, sorry.')
            ...
            result = '```Season ' + str(number) + linesep + linesep
