...
class Gambling(BaseCog):
    ...
    def extend_season_output(self, number, season_trivia_table, season_main_db, season_tables):
        ...
        if gambling['gambling_profit'] > 0:
            result += 'Most profit made from gambling'.ljust(config.season_ljust) + '  ' + str(gambling['gambling_profit']) + ' by ' + gambling['user'] + linesep + linesep
    ...
    async def unlock(self, context):
        """[ADMINS ONLY] Unlocks high-stake gambling."""
        ...
        await self.bot.post_message(self.bot.bot_channel, '**[INFO]** High-stakes gambling is now unlocked.')
        await self.bot.post_message(self.bot.bot_channel, '**[INFO]** Max points to give per day is 10000.')
    ...
    async def subscribe(self, context):
        ...
        if role in context.message.author.roles:
            ...
            await self.bot.post_message(self.bot.bot_channel, '**[INFO]** Added ' + context.message.author.name + ' to the subscriber list.')
    ...
    async def unsubscribe(self, context):
        ...
        if role not in context.message.author.roles:
            ...
            await self.bot.post_message(self.bot.bot_channel, '**[INFO]** Removed ' + context.message.author.name + ' from the subscriber list.')
