...
class TimedEvents(BaseCog):
    """A cog for timed events; e.g. holidays, free points, paying back loans."""
    ...
    def cog_unload(self):
        """Cancel timed event on cog unload."""
        self.timed_task.cancel()


def setup(bot):
    """Load timed events cog."""
    bot.add_cog(TimedEvents(bot))
    log.info("Timed events cog loaded")
